...
export async function onRequest(context) {
  ...
  return ok({ items: (rows.results || []).map((row) => ({ ...row, image_url: row.image_file_id ? `/api/media/${row.image_file_id}` : null })) }, 200, { "cache-control": "public, max-age=60, stale-while-revalidate=300" });
}
