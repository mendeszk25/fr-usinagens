...
export async function onRequest(context){
  ...
  return ok({items:(rows.results||[]).map(r=>({...r,before_url:r.before_file_id?`/api/media/${r.before_file_id}`:null,after_url:r.after_file_id?`/api/media/${r.after_file_id}`:null}))});
}
